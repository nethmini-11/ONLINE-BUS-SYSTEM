...
class EditAdminUser extends Component {
  ...
  componentDidMount() {
    let active_user_id = sessionStorage.getItem("selectedUserID:");
  ...
  async postData() {
    try {
      ...
      toast.info("✔️ User Profile Updated Susseccfully !", {
        position: "top-right",
        autoClose: 3000,
      ...
      setTimeout(
        function () {
          //Start the timer
          this.setState({ redirect: "/adminuserlist" }); //After 3 second, set redirect to true
        }.bind(this),
        3000
      );
    } catch (error) {
